...
function onOpen() {
  ...
  var menu = ui.createMenu('SMR Options');
  var item = menu.addItem('Update Tracker','SMR_Code');
  item.addToUi();
}
function SMR_Code(){
  if(DriveApp.getFilesByName("SMR.csv").hasNext() == true){
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var file = DriveApp.getFilesByName("SMR.csv").next();    
...
function MajorType(Flag){
  var Tech = {};
  var Non_Tech = {};
  var Col = ['F2:F','A2:A','B2:B','E2:E'];
  var Data = ColData(Col,"MajCode DB");
  for(var i=0; i!=Data[1].length; i++){
    Tech[Data[1][i]] = Data[2][i];
    if(Data[3][i]!=""){
      Non_Tech[Data[3][i]] = Data[0][i];
    }
  }
  if(Flag == 1){
    return Tech;
...
function ColData(GrabCol,GrabSheet){
  ...
  var Sheet = activeSpreadsheet.getSheetByName(GrabSheet);
  Data = [];
  Ranges =[];
...
function PushMajUpdates(){
  var DetColRange = [];
  var CadetRow = []; 
  var CustomColNames = ['Majcode','Major','Major Type'];
  var DetCols = FindCols(CustomColNames);
  var Tech_Dict = MajorType(1);
  var Non_Tech_Dict = MajorType(2);
  var Internal_Data = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("SMR");
  var Maj_Type = Internal_Data.getRange(DetCols[2]);
  var Major = Internal_Data.getRange(DetCols[1]);  
